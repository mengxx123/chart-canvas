...
Node.prototype.drawTip = function(ctx){
    ...
    if(textWidth > this.width){
        ...
        ctx.strokeText(tip, -this.width() + this.getWidth() - textWidth - 2, this.y - 2);
    }
    ctx.stroke()
    ctx.closePath()
    ctx.restore()
...
function UMLClassNode(name){
    ...
    node.draw = function(ctx){
        ...
        ctx.strokeText(node.className, this.x + (this.getWidth() - textWidth)/2, this.y + node.rowHeight);

        for(var i=0; i<this.operations.length; i++){
            var operation = this.operations[i]
            ctx.strokeText(operation, this.x + 5, this.y + node.rowHeight + node.rowHeight * (i+1))
        }
        ...
        for(var i=0; i<this.attributes.length; i++){
            ...
            ctx.strokeText(attribute, this.x + 5, begingHeight + this.rowHeight  * (i+1))
        }
        ctx.stroke()
        ctx.closePath()
        ctx.restore()
